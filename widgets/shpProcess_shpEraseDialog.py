...
from ui.shp_process_dialog.shpEraseDialog import Ui_shpEraseDialog
...
class ShpEraseDialogClass(Ui_shpEraseDialog,QDialog):
    ...
    def initUI(self):
        ...
        for layer in PROJECT.mapLayers().values():
            ...
            if layer.type() == QgsMapLayerType.VectorLayer and layer.geometryType() == QgsWkbTypes.PolygonGeometry:
                ...
                self.selectEraseShpCb.addItem(layer.name())
    ...
    def runPBClicked(self):
        ...
        if self.selectEraseShpCb.count() == 0:
            MessageBox(self.yoyiTrs._translate('警告'), self.yoyiTrs._translate('没有有效矢量'), self).exec_()
            return
        ...
        currentEraseLayerName = self.selectEraseShpCb.currentText()
        currentEraseLayer : QgsVectorLayer = PROJECT.mapLayersByName(currentEraseLayerName)[0]

        if currentLayerName == currentEraseLayerName:
        ...
        self.erasePath = currentEraseLayer.source()
